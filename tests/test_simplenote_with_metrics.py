...
class TestSimplenoteWithMetrics(unittest.TestCase):
    ...
    def test_simplenote_auth(self):
        token = Simplenote(self.user, self.password, True).get_token()
    ...
    def test_simplenote_get_list_length(self):
        res, status = Simplenote(self.user, self.password, True).get_note_list()
        if status == 0:
    ...
    def test_simplenote_get_list_length_25(self):
        for i in xrange(25):
            note, status = Simplenote(self.user, self.password, True).add_note("Note "+str(i))
            if status == 0:
                self.initial_note_count += 1
    ...
    def test_simplenote_limit_list_length_21(self):
        for i in xrange(25):
            note, status = Simplenote(self.user, self.password, True).add_note("Note "+str(i))
        res, status = Simplenote(self.user, self.password, True).get_note_list(21)
        if status == 0:
    ...
    def test_simplenote_first_note(self):
        if self.first_note != False:
            note, status = Simplenote(self.user, self.password, True).get_note(self.first_note)
            if status == 0:
    ...
    def test_simplenote_second_note(self):
        if self.second_note != False:
            note, status = Simplenote(self.user,
                                    self.password, True).get_note(self.second_note)
            if status == 0:
                self.assertTrue(type(note) == dict)
    ...
    def test_simplenote_trash_note(self):
        if self.first_note != False:
            note, status = Simplenote(self.user,
                                    self.password, True).trash_note(self.first_note)
            if status == 0:
                self.assertEqual(1, note["deleted"])

        if self.second_note != False:
            note, status = Simplenote(self.user,
                                    self.password, True).trash_note(self.second_note)
            if status == 0:
                self.assertEqual(1, note["deleted"])
    ...
    def test_simplenote_delete_note(self):
        if self.first_note != False:
            note, status = Simplenote(self.user,
                                    self.password, True).delete_note(self.first_note)
            if status == 0:
                note, status = Simplenote(self.user,
                                          self.password, True).get_note(self.first_note)
                self.assertEqual(-1, status)

        if self.second_note != False:
            note, status = Simplenote(self.user,
                                    self.password, True).delete_note(self.second_note)
            if status == 0:
                note, status = Simplenote(self.user,
                                          self.password, True).get_note(self.second_note)
                self.assertEqual(-1, status)
    ...
    def test_simplenote_add_note_object(self):
        res, status = Simplenote(self.user, self.password, True).add_note({"content":
                                                                     "new note"})
        if status == 0:
            note, status = Simplenote(self.user, self.password, True).get_note(res["key"])
            if status == 0:
                self.assertEqual("new note", note["content"])

    def test_simplenote_add_note_content(self):
        res, status = Simplenote(self.user, self.password, True).add_note("new note")
        if status == 0:
            note, status = Simplenote(self.user, self.password, True).get_note(res["key"])
            if status == 0:
                self.assertEqual("new note", note["content"])
    ...
    def test_simplenote_update_note(self):
        ...
        note, status = Simplenote(self.user, self.password, True).update_note(note)
        if status == 0:
            note, status = Simplenote(self.user, self.password, True).get_note(note["key"])
            if status == 0:
    ...
    def test_simplenote_is_unicode(self):
        if self.unicode_note_key != False:
            note, status = Simplenote(self.user,
                                    self.password, True).get_note(self.unicode_note_key)
            if status == 0:
                self.assertTrue(self.is_utf8(note["content"]))

    def test_note_with_plus_signs(self):
        note, status = Simplenote(self.user, self.password, True).add_note("++")
        if status == 0:
            note, status = Simplenote(self.user, self.password, True).get_note(note["key"])
            if status == 0:
    ...
    def clear_all_notes(self):
        res, status = Simplenote(self.user, self.password, True).get_note_list()
        if status == 0:
            [Simplenote(self.user, self.password, True).delete_note(n["key"]) for n in res]
